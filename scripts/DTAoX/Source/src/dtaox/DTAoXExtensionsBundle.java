...
import ro.sync.exml.workspace.api.*;
...
public class DTAoXExtensionsBundle extends ro.sync.ecss.extensions.api.ExtensionsBundle{
	...
	public DTAoXExtensionsBundle() {
	    //disable caret highlighting in oxygen preferences, only available since Oxygen 15
		PluginWorkspace workspace = PluginWorkspaceProvider.getPluginWorkspace();
		if(Double.valueOf(workspace.getVersion())>=15)
			workspace.setGlobalObjectProperty("author.show.caret.proximity.highlight", Boolean.FALSE);
	}
